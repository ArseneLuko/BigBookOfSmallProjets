...
try:
    ...
    pass # if pyperclip not installed, do nothing. It's not big deal :-)

# Every possible symbol that can be encrypted/decrypted:
# (!) You can add numbers and punctation marks to encrypt those
# symbols as well
...
while True:
    ...
    if response.startswith('e'):  # Lukáš: použití startswith rozšíří odpovědi
        mode = 'encrypt'
        break
...
for symbol in message:
    if symbol in SYMBOLS:
        # Get the encrypted (or decrypted) number fot this symbol.
        ...
        # Add encrypted(decrypted) number's symbol to tranlated
        translated += SYMBOLS[num]
    else:
        # Just add the symbol without encrypting/dwecrypting
        translated += symbol
